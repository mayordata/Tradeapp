...
### How it works:
1. Select a futures instrument from the dropdown list.
...
# Dropdown to select a futures instrument with placeholder (no pre-selected instrument)
selected_instrument = st.selectbox("Select a Futures Instrument:", ["--Select an Instrument--"] + list(futures_data.keys()), key="instrument")

# Check if an instrument has been selected (no pre-selection)
if selected_instrument != "--Select an Instrument--":
    # Get selected instrument data
    instrument_data = futures_data[selected_instrument]

    # Display instrument details after selection
    ...
    # Input fields for the user (No default values, allowing user input)
    opening_profit = st.number_input("Enter the opening profit before the trade (in dollars):", min_value=0.0, step=1.0, key="opening_profit")
    risk_amount = st.number_input("Enter the amount you are willing to risk (in dollars):", min_value=0.0, step=1.0, key="risk_amount")
